...
public class Student implements Cloneable{
    ...
    public int compareName(Student compareStudent){
        if(this.lN.compareTo(compareStudent.lN) > 0){
            return 1;
        }else if(this.lN.compareTo(compareStudent.lN) < 0){
            return -1;
        }else{
            if(this.fN.compareTo(compareStudent.fN) > 0){
                return 1;
            }else if(this.fN.compareTo(compareStudent.fN) < 0){
                return -1;
            }else{
                return compareAverage(compareStudent);
            }
        }
    }

    public int compareAverage(Student compareStudent){
        //System.out.println("Comparing by the Average");
        if(this.getAverage() < compareStudent.getAverage()){
            return 1;
        }else if(this.getAverage() > compareStudent.getAverage()){
            return -1;
        }else return 0;
    }
}
